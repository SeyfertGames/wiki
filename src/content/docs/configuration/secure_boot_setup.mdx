...
3. Enroll your keys with Microsoft's and the OEM firmware's built-in keys:
    ...
    :::caution[ASUS/Lenovo Motherboards — Do not use `--firmware-builtin`]
    On some ASUS/Lenovo motherboards using the `--firmware-builtin` flag causes duplicate entries
    ...
    Use only `--microsoft` on ASUS/Lenovo boards:
